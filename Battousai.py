...
nadya = LINETCR.LINE()
#nadya.login(qr=True)
nadya.login(token='ErtfLkd5GNPMM9Yyqq33.xGIo4dpZc6G2+/j2UfXQKW.SGT+czl2kHlKmCRCASrerPwbzGyzGHvoN2aL12u0zE4=')
nadya.loginResult()
...
╠➩〘Urlgroup Image〙
╠═════════════════════════
║      ⚛️ By :F҉bL҉ V҉ic҉ky҉҉ ⚛️
╚═════════════════════════
"""
tjia="u91b4d82e07fbb74ec6d87f1d620f3b8c"
...
║╠═══════════════
║╠ ☠️ BOT ☠️
║╠􀌂􀄠􏿿􀌂􀄠{﷽} ✍ Allprotect on/ff
║╠􀌂􀄠􏿿􀌂􀄠{﷽} ✍️〘Tag all〙
║╠{﷽} ✍〘Sider on/off〙
║╠{﷽} ✍〘Gift1 @ s/d Gift10 @〙
║╠════════════════
║╠  🎃✝️ BOT VERSION ✝️🎃
║╠ ✍ ️Version: V.3 red Check Mark 
║╠ ✍ ️System: Python 3
║╠ ✍️ Jumlah: Bot 10 Asist
║╚════════════════
╠═══════════════════════
║ ⚛️ By :F҉bL҉ V҉ic҉ky҉҉ ⚛️
╚═══════════════════════
...
Bots=["u5e088ee033f30ba836ffe276b85c6a93"]
Creator=["u5e088ee033f30ba836ffe276b85c6a93"]
admin=["u5e088ee033f30ba836ffe276b85c6a93"]
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"Bot Auto Like ©By :⚛️ By :F҉bL҉ V҉ic҉ky҉҉ ⚛️ .",    
    "commentOn":True,
    "commentBlack":{},
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = nadya.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Ngapain Tag Tag! Kangen ya 😛","Ngetag Mele 😴 Orang Cakep mah Banyak Fans","Tampol ni ngetag trus " + cName + " 😜 Yg penting Sombong!"]
                    ret_ = random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = nadya.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Ngapain Tag" + cName + ", Kyk Gk pernah liyat orang Cakep aja😜!"]
                    balas1 = "Orang Jelek dilarang Tag. . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
